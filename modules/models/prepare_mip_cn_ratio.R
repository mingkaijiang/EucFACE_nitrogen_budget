prepare_mip_cn_ratio <- function(cnDF,
                                 ambDF,
                                 eleDF) {
    ...
    ### model based results
    ambDF$canopy <- with(ambDF, CL.mean/NL.mean)
    ambDF$leaflitter <- with(ambDF, CFLITA.mean/NFLITA.mean)
    ambDF$wood <- with(ambDF, CW.mean/NW.mean)
    ambDF$fineroot <- with(ambDF, CFR.mean/NFR.mean)
    ambDF$soil_0_10 <- with(ambDF, CSOIL.mean/NSOIL.mean)

    eleDF$canopy <- with(eleDF, CL.mean/NL.mean)
    eleDF$leaflitter <- with(eleDF, CFLITA.mean/NFLITA.mean)
    eleDF$wood <- with(eleDF, CW.mean/NW.mean)
    eleDF$fineroot <- with(eleDF, CFR.mean/NFR.mean)
    eleDF$soil_0_10 <- with(eleDF, CSOIL.mean/NSOIL.mean)
