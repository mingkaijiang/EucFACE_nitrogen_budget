make_seedlitter_n_flux <- function(n_conc, litter_flux) {
    ...
    outDF$seedlitter_n_flux_mg_m2_d <- outDF$seed_flux*outDF$PercN/100
    
    outDF$Days <- as.numeric(with(outDF, End_date - Start_date))
    
    outDF <- outDF[,c("Date", "Start_date", "End_date", "Ring", "seedlitter_n_flux_mg_m2_d", "Days")]
