...
make_soil_n_nitrification_flux <- function(bk_density) {
    ...
    download_soil_n_nitrification_flux()
    ...
    myDF.m <- summaryBy(nitrification~date+ring,data=myDF,FUN=mean,keep.names=T,na.rm=T)
    ...
    myDF.m$soil_n_nitrification <- myDF.m$nitrification * myDF.m$bk_density * 0.1 
